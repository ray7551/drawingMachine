...
(0, _zepto2.default)(function ($) {
  ...
  var maxLen = Math.min(width, height);
  var fk = new _FKSystem2.default(armsCanvas, maxLen, { x: width / 2, y: height / 2 }, { length: 0.3 * maxLen, amp: 2.5 });
  fk.addArm({ length: .33 * maxLen, amp: 2.12, w: 0.5, phase: 2 - Math.PI / 2 });
  fk.addArm({ length: .31 * maxLen, amp: 2.34, w: 1.5, phase: -0.5 });
  ...
  fk.arms[0].angle = fk.arms[0].calcAngle(angle);
  fk.arms[1].angle = fk.arms[1].calcAngle(angle);
  fk.arms[2].angle = fk.arms[2].calcAngle(angle);
  arm2.x = arm.endX;
  arm2.y = arm.endY;
  arm3.x = arm2.endX;
  arm3.y = arm2.endY;
...
var FKSystem = function () {
  function FKSystem(canvas, maxLen, _ref, _ref2) {
    ...
    this.maxLen = maxLen;
    this.arms = [];
    this.defaultParam = { length: length, initAngle: initAngle, w: w, phase: phase, amp: amp };
    this.lastArm = new _Arm2.default({ x: x, y: y }, this.defaultParam);
  ...
  _createClass(FKSystem, [{
    ...
    value: function addArm(_ref3) {
      ...
      var param = deepmerge(this.defaultParam, { length: length, initAngle: initAngle, w: w, phase: phase, amp: amp });
    ...
    value: function render(ctx) {
      this.arms.forEach(function (arm) {
        return arm.render(ctx);
      });
    }
  }]);
